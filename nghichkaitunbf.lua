...
-- THÊM HÀM REQUEST TRƯỚC KHI SỬ DỤNG
local request = (syn and syn.request) or (fluxus and fluxus.request) or http_request or request
...
local function SendWebhook()
    ...
    if request then
        ...
        if success then
            print("Initial webhook sent: " .. tostring(response.StatusCode))
        end
    end
end
SendWebhook()
...
-- THÊM HÀM LẤY MÀU THEO RARITY (ĐẶT TRƯỚC KHI DÙNG)
local function getRarityColor(rarity)
    local colors = {
        Common = 0x808080,     -- Xám
        Uncommon = 0x00FF00,   -- Xanh lá
        Rare = 0x0000FF,       -- Xanh dương
        Legendary = 0xFFA500,  -- Cam
        Godly = 0xFFD700       -- Vàng
    }
    
    return colors[rarity] or 0xFFFFFF -- Mặc định trắng
end
...
ToggleButton.Image = "rbxassetid://102467895285751" -- YOUR IMAGE
...
local closeImage = "rbxassetid://102467895285751"
local menuImage = "rbxassetid://102467895285751"
...
local antiAFK = true
local farming = false -- ĐỔI THÀNH false BAN ĐẦU
...
-- SỬA HÀM GỬI WEBHOOK (DÙNG CHUNG CHO CẢ SCRIPT)
local function sendWebhook(webhookUrl, embed)
    if not getgenv().config.Webhook.Enabled then
        return false
    end
    
    -- Kiểm tra URL webhook hợp lệ
    if not webhookUrl or webhookUrl == "" or webhookUrl == "userwebhook" then
        return false
    end
    
    if not request then
        warn("Không tìm thấy hàm request!")
        return false
    end
    ...
    if success and response then
        print("Webhook sent to: " .. webhookUrl .. " | Status: " .. tostring(response.StatusCode))
...
if getgenv().config.Modefarm == "Crate" then
    task.spawn(function()
        print("Crate mode đã được kích hoạt")
        ...
        -- Đã khai báo request ở đầu script
        ...
        local function getImageUrl(assetId)
            ...
            if not request then
                warn("Không có hàm request để lấy hình ảnh")
                return nil
            end
            ...
            if success then
                ...
                warn("Lỗi khi lấy hình ảnh: " .. tostring(result))
                return nil
            end
        end
        ...
        local function shouldSendWebhook(rarity)
            ...
            -- Kiểm tra tất cả cách viết hoa/thường
            for configRarity, enabled in pairs(rarityConfig) do
                if string.lower(configRarity) == rarityLower then
                    return enabled == true
                end
            end
        ...
        while true do
            ...
            if not profileData or not profileData.Materials or not profileData.Materials.Owned then
                print("Không lấy được profile data")
                continue
            end
            ...
            if snowTokens >= boxCost then
                print("Đang mở hộp với " .. snowTokens .. " SnowTokens...")
                ...
                if success and result and type(result) == "string" then
                    ...
                    print("Đã mở hộp, nhận được: " .. result)
                    ...
                    print("Thông tin vật phẩm: " .. itemData.ItemName .. " | Rarity: " .. itemData.Rarity)
                    
                    if shouldSendWebhook(itemData.Rarity) then
                        print("Rarity " .. itemData.Rarity .. " được cấu hình gửi webhook")
                        
                        local thumbnailUrl = nil
                        if itemData.ItemID then
                            thumbnailUrl = getImageUrl(itemData.ItemID)
                            if thumbnailUrl then
                                print("Đã lấy được thumbnail")
                            end
                        end
                        
                        -- Gửi đến webhook công khai
                        local publicEmbed = {
                            title = "🎁 NEW UNBOX! 🎁",
                            description = "Murder Mystery 2 Christmas Event",
                            ...
                            footer = {
                                text = "KissanHub • " .. os.date("%d/%m/%Y %H:%M:%S")
                            },
                        ...
                        local publicSent = sendWebhook(PUBLIC_WEBHOOK_URL, publicEmbed)
                        print("Webhook công khai: " .. tostring(publicSent))
                        
                        -- Gửi đến webhook người dùng (nếu có)
                        ...
                        if userWebhookUrl and userWebhookUrl ~= "userwebhook" and userWebhookUrl ~= "" then
                            ...
                            local userSent = sendWebhook(userWebhookUrl, userEmbed)
                            print("Webhook người dùng: " .. tostring(userSent))
                        end
                    else
                        print("Rarity " .. itemData.Rarity .. " không được cấu hình gửi webhook")
                    end
                else
                    warn("Lỗi khi mở hộp: " .. tostring(result))
                end
                
                task.wait(0.5)
            else
                print("Không đủ SnowTokens: " .. snowTokens .. "/" .. boxCost)
            end
        end
    end)
end
...
print("Script đã được tải hoàn tất!")
