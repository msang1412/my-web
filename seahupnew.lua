if getgenv().Kissan then 
	if game.CoreGui:FindFirstChild("Kissan Hub GUI") then for i, v in ipairs(game.CoreGui:GetChildren()) do if string.find(v.Name,  "Kissan Hub") then v:Destroy() end end end
end
getgenv().Kissan = true
...
local T1UIColor = {
	...
	["Logo Image"] = "http://www.roblox.com/asset/?id=134369701407905",
...
local IndexUIColor = {
	...
	["Click Effect Color"] = Color3.fromRGB(230, 230, 230),
	["Setting Icon Color"] = Color3.fromRGB(230, 230, 230),
	["Logo Image"] = "rbxassetid://134369701407905",
	["Search Icon Color"] = Color3.fromRGB(255, 255, 255),
...
Library_Function.Gui.Name = 'Kissan Hub GUI'
...
Library_Function.NotiGui.Name = 'Kissan Hub Notification'

Library_Function.HideGui = Instance.new('ScreenGui')
Library_Function.HideGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Library_Function.HideGui.Name = 'Kissan Hub Btn'


local btnHide = Instance.new('TextButton', Library_Function.HideGui) 
btnHide.BackgroundTransparency = 1
btnHide.Text = ""
btnHide.AnchorPoint = Vector2.new(0, 0)  -- â¬…ï¸ Äá»•i anchor point
btnHide.Size = UDim2.new(0,50,0,50)
btnHide.Position = UDim2.new(0,15,0,15)  -- â¬…ï¸ Äá»•i position (gĂ³c trĂªn trĂ¡i)

local btnHideFrame = Instance.new('Frame', btnHide)
btnHideFrame.AnchorPoint = Vector2.new(0, 0)  -- â¬…ï¸ Äá»•i anchor point
btnHideFrame.Size = UDim2.new(0,50,0,50)
btnHideFrame.Position = UDim2.new(0,0,0,0)  -- â¬…ï¸ Äá»•i position
...
Library.ToggleUI = function()
	...
	if game.CoreGui:FindFirstChild("Kissan Hub GUI")then for a,b in ipairs(game.CoreGui:GetChildren())do if b.Name=="Kissan Hub GUI"then b.Enabled= getgenv().UIToggled end end end
end
...
local libCreateNoti = function(Setting)
	...
	TextLabelNoti.Text = "<font color=\"rgb(" .. color .. ")\">Kissan Hub</font> "..getgenv().TitleNameNoti
...
function Library.CreateMain(Setting)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
	TextLabelMain.Text = "<font color=\"rgb("..color..")\">Kissan Hub</font> " .. getgenv().MainDesc
...
SettingsButton.MouseButton1Click:Connect(function()
		...
		local pageFunction = {}
			...
			function pageFunction.CreateSection(Section_Name, Toggleable, SectionGap, SectionColor)
				...
				local sectionFunction = {}
				 ...
				 bar (tùy chọn)
				lfunction sectionFunction.CreateButtonNew(Setting, Callback)
	local Title = Setting.Title or "Button"
	local SubTitle = Setting.SubTitle or ""
	local Icon = Setting.Icon or "rbxassetid://3926307971"
	local ButtonText = Setting.ButtonText or "RUN"
	local Callback = Callback or function() end
	
	-- ===== CONTAINER CHÍNH (CÙNG KÍCH THƯỚC BUTTON CŨ) =====
	...
	MainContainer.Size = UDim2.new(1, 0, 0, 25) -- BẰNG BUTTON CŨ
	...
	Background.Size = UDim2.new(1, -10, 1, 0)
	Background.BackgroundColor3 = getgenv().UIColor["Background 1 Color"]
	...
	-- ===== PHẦN TEXT BÊN TRÁI (80% width) =====
	TextSide.Name = "TextSide"
	TextSide.Parent = Background
	TextSide.BackgroundTransparency = 1
	TextSide.Position = UDim2.new(0, 10, 0, 0)
	TextSide.Size = UDim2.new(0.8, -15, 1, 0)
	...
	-- Không dùng subtitle vì button quá nhỏ
	
	-- ===== BUTTON ACTION BÊN PHẢI (20% width, TRÒN) =====
	...
	ActionSide.Position = UDim2.new(1, -5, 0.5, 0)
	ActionSide.Size = UDim2.new(0.2, 0, 0, 20)
	
	-- Button TRÒN (hình tròn)
	...
	local hoverColor = Color3.new(
		math.clamp(originalColor.R * 1.3, 0, 1),
		math.clamp(originalColor.G * 1.3, 0, 1),
		math.clamp(originalColor.B * 1.3, 0, 1)
	)
	
	local originalSize = ActionButton.Size
	local hoverSize = UDim2.new(1.15, 0, 1.15, 0) -- Phóng to ít hơn
	...
	ActionHitbox.MouseButton1Down:Connect(function()
		...
		TweenService:Create(ActionButton, TweenInfo.new(0.1), {
			BackgroundColor3 = Color3.new(
				math.clamp(originalColor.R * 0.6, 0, 1),
				math.clamp(originalColor.G * 0.6, 0, 1),
				math.clamp(originalColor.B * 0.6, 0, 1)
			),
		...
		-- Hiệu ứng thả
		TweenService:Create(ActionButton, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			BackgroundColor3 = originalColor,
			Size = originalSize
		}):Play()
		
		TweenService:Create(ActionIcon, TweenInfo.new(0.15), {
			Size = UDim2.new(0, 14, 0, 14)
		}):Play()
	...
	function newButtonFunctions:Pulse()
		-- Hiệu ứng nhấp nháy
		for i = 1, 3 do
